"""Closed-loop GR00T evaluation for simple pick-and-place task.

Loads the built-in Isaac Lab env for scene/physics/camera, runs GR00T inference,
and directly sets joint position targets on the robot (bypassing PINK IK action manager).

Usage:
    cd /home/ray/IsaacLab-Arena/submodules/IsaacLab
    ./isaaclab.sh -p ../../isaaclab_arena_gr00t/eval_gr00t_simple_pick_place.py \
        --model_path /home/ray/models/simple-pick-and-place-merged \
...
from isaaclab_arena_gr00t.data_config import UnitreeG1FixedBaseDataConfig
...
# Register envs & import scene/obs/termination configs (NOT the full env cfg which needs PINK IK)
import isaaclab_tasks  # noqa: F401
import isaaclab_tasks.manager_based.locomanipulation.pick_place  # noqa: F401
from isaaclab_tasks.manager_based.locomanipulation.pick_place.simple_pick_place_g1_env_cfg import (
    SimplePickPlaceG1SceneCfg,
    ObservationsCfg,
    TerminationsCfg,
)
...
class EvalActionsCfg:
    """Direct joint position control — bypasses PINK IK entirely."""
...
@configclass
class EvalEnvCfg(ManagerBasedRLEnvCfg):
    """Eval env config: same scene/obs/terminations but with direct joint control."""
    scene: SimplePickPlaceG1SceneCfg = SimplePickPlaceG1SceneCfg(
...
def build_groot_obs(joint_pos_sim, camera_rgb, policy_joints_config, sim_state_joints_config, num_envs, language):
    ...
    obs = {
        ...
        "state.waist": joint_pos_policy["waist"].reshape(num_envs, 1, -1),
...
def main():
    ...
    # Load GR00T policy
    data_config = UnitreeG1FixedBaseDataConfig()
    ...
    language = "Pick up the brown box and place it on the green target square."
    num_envs = 1
    device = "cuda"
        ...
        for step in range(args.max_steps):
            ...
            if action_chunk is None or action_idx >= args.action_horizon:
                ...
                if step == 0 and ep == 0:
                    ...
                    diff = (action_chunk[0, 0, :] - joint_pos[0, :].cpu()).abs()
            ...
            if step < 3 and ep == 0:
                ...
                print(f"  [DEBUG] step={step} waist: before={jp_before[12:15].tolist()} target={tp[12:15].tolist()} after={jp_after[12:15].tolist()}")
                print(f"  [DEBUG] step={step} rarm:  before={jp_before[29:36].tolist()} target={tp[29:36].tolist()} after={jp_after[29:36].tolist()}")
