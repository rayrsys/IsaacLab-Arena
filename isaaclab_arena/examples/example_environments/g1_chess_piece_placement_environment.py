...
class G1ChessPiecePlacementEnvironment(ExampleEnvironmentBase):

    name: str = "g1_chess_piece_placement"
    ...
    def get_env(self, args_cli: argparse.Namespace):
        ...
        # The hand tracking retargeters output actions compatible with PinkInverseKinematicsActionCfg
        # but NOT with Arena's G1DecoupledWBCPinkActionCfg (different action dimensions).
        if args_cli.teleop_device is not None and "handtracking" in args_cli.teleop_device.lower():
            ...
            # Set the URDF path required by the PINK IK controller
        ...
        # Chess board on top of the table (visual only)
        chess_board.set_initial_pose(
            Pose(position_xyz=(0.5, 0.0, 0.6), rotation_wxyz=(1.0, 0.0, 0.0, 0.0))
        )

        # Chess piece starting position (on the table, reachable by robot)
        chess_piece.set_initial_pose(
            Pose(position_xyz=(0.5, 0.1, 0.52), rotation_wxyz=(1.0, 0.0, 0.0, 0.0))
        )

        # Target square (destination, ~20cm from chess piece)
        chess_target_square.set_initial_pose(
            Pose(position_xyz=(0.5, -0.1, 0.51), rotation_wxyz=(1.0, 0.0, 0.0, 0.0))
        )

        scene = Scene(assets=[GroundPlane(), DomeLight(), office_table, chess_board, chess_piece, chess_target_square])
        isaaclab_arena_environment = IsaacLabArenaEnvironment(
            name=self.name,
            embodiment=embodiment,
            scene=scene,
            task=G1ChessPiecePlacementTask(chess_piece, chess_target_square, episode_length_s=30.0),
